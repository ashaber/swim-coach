...
TAPER_WEEKS_LONG = 4
TAPER_WEEKS_SHORT = 2
# 4-week taper for runways >= 16 weeks, per ROADMAP.md "Research sources"
# [Source 01] (Formosa et al., 78km solo OW case study: "4-week exponential
# decay model. Volume reduced by 25% linearly each week"). Shorter runways
# compress to a 2-week taper -- PROVISIONAL, library/03-periodization.md.
...
# 4-week, 25%-of-peak-per-week linear taper decay, per ROADMAP.md
# "Research sources" [Source 01] (Formosa et al. 78km case study).
...
# PROVISIONAL: Saturday long swim as a share of that week's target volume.
# library/06-long-swim-progression.md (to be authored).
...
def generate_week(
    athlete: Athlete, macro: MacroPlan, iso_week: str, week_start: date
) -> WeekPlan:
    ...
    Sessions emitted:
      ...
      - one long open-water swim on Saturday (LONG_SWIM_SHARE of weekly
        target, capped during taper -- see below)
      - STRENGTH_SESSIONS_PER_WEEK strength sessions, placed on days
        without pool practice where possible
      - one recovery/mobility day (Sunday)
    ...
    directly to the long swim regardless of what the general linear
    weekly-target interpolation computes for that week.
    """
