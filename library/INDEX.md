...
| `10-recovery-hrv.md` | Recovery-science support for `adapt.py`'s post-milestone recovery window and the `wellness_composite` subjective-primary-signal design; sleep as the highest-leverage lever; refeed-window carbohydrate/protein; CWI/compression comfort-vs-performance tiering; HRV-guided load-adjustment evidence (forward-looking — device exists in profile, no HRV data logged yet); swim-specific sRPE-over-ACWR recovery monitoring (cross-refs `03`); a between-events mini-taper evidence base with the "no direct evidence for two ultra swims ~9 days apart" gap stated plainly. **UNREVIEWED**, pending human review. |
...
authored after Day 4 and are `UNREVIEWED` pending human review. Day 4
