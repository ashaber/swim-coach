    ...
    "items": {
        ...
        "properties": {
            ...
            "interval_type": {
                "type": "string",
                "description": (
                    "Bike sessions only: switch this ride to one of the engine's deterministic interval "
                    "types -- threshold, over_unders, vo2, race_pace, openers -- or back to 'endurance' "
                    "(easy Z2). The ENGINE builds the intervals, zone tag and prose (keeps the date, length "
                    "and the ride's name), so you never hand-author a workout to change a ride's target: "
                    "e.g. 'make Sunday's group ride a threshold session this week'. Add `purpose` to override "
                    "the text."
                ),
            },
            "intensity": {
...
TOOLS_SCHEMA: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            "THE tool for stating ANY schedule preference -- there is no other. Save the SHAPE of the "
            "athlete's training week (which sessions on which days) so every future build/base week is "
            "built from it: a whole week ('Monday CX skills and yoga, Tuesday intervals then strength, "
            "Wednesday group ride, Thursday off, Friday yoga, Saturday intervals and strength, Sunday "
            "group ride') OR a single standing preference ('Wed and Sun club rides', 'Tuesday and "
            "Saturday are my interval days', 'strength the same day after intervals'). NEVER "
            "hand-author session_overrides week after week to reproduce a pattern the athlete keeps "
            "asking for. There is NO limit on interval days or session types. "
            "`template`: {mon..sun: [slot, ...]} -- a day left out or [] is a day OFF. A slot is "
            "{kind, role?, intervals?, label?, duration_min?, purpose?, structure?}. kind 'bike': role "
            "'hard' (an interval ride; any number per week; `intervals` picks the engine's interval "
            "type -- threshold, over_unders, vo2, race_pace, openers -- or omit it for the weekly "
            "rotation), 'endurance' (a Z2 ride), or 'flex' (a ride that can be EITHER easy or pushed, "
            "e.g. a group ride: built as Z2 with the optional push described; never counts as a hard "
            "day; switch it per week with the `interval_type` override). Put a club ride's name in "
            "`label` and its usual length in `duration_min`. Other kinds: 'skills' (cyclocross skills), "
            "'strength' (put it AFTER the hard ride in the day's list to say 'after the intervals'), "
            "'yoga', 'recovery' -- and ANY other kind (swim, run, row, hike, core, kettlebell ...): "
            "accepted and written as described, never refused. Put the athlete's own words in `purpose` "
            "(short) and `structure` (full instructions) so their content is used as written. Anything "
            "unusual (an unrecognised role, over-long text, an out-of-range duration) is fixed up and "
            "reported in `warnings`, not rejected. "
            "`clear: true` removes the template. ALWAYS call it WITHOUT `confirm` first, read the "
            "returned `week` grid and any `warnings` back to the athlete, and only call again with "
            "`confirm: true` after they agree in a new message. It does not change weeks already on "
            "file -- rebuild those with replace_week_plan's usual draft-then-confirm. Taper and race "
            "weeks ignore it (the engine's taper/race placement wins, and says so). Volume comes from "
            "the macro's ramp-capped target; the template never sets load."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                ...
                                "role": {
                                    "type": "string",
                                    "description": (
                                        "bike slots only: hard (intervals/tempo/threshold/vo2), endurance "
                                        "(easy/long/group), or flex (either easy or pushed). Anything unrecognised "
                                        "becomes endurance, with a note."
                                    ),
                                },
                                "intervals": {
                                    "type": "string",
                                    "description": (
                                        "hard or flex bike slots: which of the engine's interval types this ride "
                                        "uses -- threshold, over_unders, vo2, race_pace, openers. Omit for the weekly "
                                        "rotation (hard) or a threshold push (flex)."
                                    ),
                                },
                                "sport": {
                                    "type": "string",
...
def _handle_get_plan_summary(input_data: dict[str, Any], *, store: StoreInterface, slug: str) -> dict[str, Any]:
    ...
    try:
        ...
        log.warn("swallowed exception, using a default", where='backend/app/tools.py', line_hint=2894, exc_info=True)
        macro = None
...
def _handle_flag_for_coach_review(
    ...
    try:
        ...
        log.warn("swallowed exception, using a default", where='backend/app/tools.py', line_hint=2931, exc_info=True)
...
def _handle_record_health_status(
    ...
    try:
        ...
        log.warn("swallowed exception, using a default", where='backend/app/tools.py', line_hint=3056, exc_info=True)
...
def _handle_record_threshold_test(
    ...
    try:
        ...
        log.error("storage read failed", what='athlete_id for slug', exc_info=True)
    ...
    for day in _TEMPLATE_DAY_ORDER:
        ...
        for slot in template.get(day, []):
            ...
            if kind == "bike":
                text = (
                    f"bike: {slot['role']}"
                    + (f" ({slot['intervals']})" if slot.get("intervals") else "")
        ...
        result.update(
            {
                ...
                "warnings": _safe_realism(nominal) + template_normalization_notes(candidate.weekly_template),
                "applies_to": (
...
def _handle_get_workouts(input_data: dict[str, Any], *, store: StoreInterface, slug: str) -> dict[str, Any]:
    ...
    if matched:
        ...
        for w in matched:
            try:
                summarized.append(_summarize_workout(w, athlete=athlete, hr_max=hr_max, wellness=wellness))
            except Exception:  # noqa: BLE001 - one odd workout must not hide the rest
                log.error("optional step failed", step="workout summary", workout_id=str(w.id), exc_info=True)
                summarized.append({
                    "id": str(w.id), "date": w.date.isoformat(), "sport": w.sport,
                    "summary_error": "this workout could not be summarized (logged); the other workouts are shown",
                })
    else:
        summarized = []
...
def _handle_export_zwo_workout(input_data: dict[str, Any], *, store: StoreInterface, slug: str) -> dict[str, Any]:
    ...
    try:
        ...
        log.warn("swallowed exception, using a default", where='backend/app/tools.py', line_hint=4296, exc_info=True)
...
def _confirm_macro_from_draft(
    ...
    try:
        ...
        log.warn("swallowed exception, using a default", where='backend/app/tools.py', line_hint=4745, exc_info=True)
        current = None
...
_REALISM_UNAVAILABLE = (
    "The realism check could not run (an internal problem, logged), so this week's hard-day count and "
    "volume ramp were NOT checked -- review them yourself before telling the athlete the plan is sound."
)


def _safe_realism(sessions: Any, **kwargs: Any) -> list[str]:
...
def _held_bike_minutes(store: StoreInterface, slug: str, week_start: date) -> float | None:
    ...
    for back in (7, 14):
        ...
        try:
            ...
            log.warn("swallowed exception, using a default", where='backend/app/tools.py', line_hint=5524, exc_info=True)
            existing = None
        if existing is None:
            continue
        minutes = sum(
...
def _apply_session_overrides(
    ...
    for override in overrides:
        ...
        if override.get("add") and len(matches) == 1 and not override.get("remove"):
            # `add` onto a date that already has a session of that sport used to be an error (it
            # blocked a write in production, 2026-09-20). Flag-don't-block: edit the existing session.
            notes.append(
                f"{raw_date}: a {matches[0].sport} session already existed that date, so this edited it "
                "instead of adding a second one"
            )
            override = {k: v for k, v in override.items() if k != "add"}
        if override.get("add"):
            ...
            if add_structure is None and add_structured is not None:
                ...
                except Exception:  # noqa: BLE001 - the prose is a convenience; the session must still be written
                    log.error("optional step failed", step="workout prose render", exc_info=True)
                    add_structure = "\n".join(_salvage_prose(add_structured_raw)) or None
                    notes.append(f"{raw_date}: the workout's text could not be rendered (logged), so its raw steps were saved as plain text")
        ...
        interval_type = override.get("interval_type")
        if (
            distance_m is None
            ...
            and interval_type is None
        ):
            return (
                f"session_overrides: entry for {raw_date!r} needs at least one of "
                "distance_m, duration_min, purpose, structure, structured, ow_template, intensity, interval_type"
            ), notes
        ...
        if interval_type is not None:
            wanted = "endurance" if str(interval_type).strip().lower() in ("endurance", "easy", "z2", "zone 2", "recovery ride") else normalize_interval_type(interval_type)
            if session.sport != "bike":
                notes.append(f"{raw_date}: interval_type only applies to bike sessions, so it was ignored for this {session.sport} session")
            elif wanted is None:
                notes.append(
                    f"{raw_date}: interval_type {interval_type!r} is not one of the engine's interval types "
                    "(threshold, over_unders, vo2, race_pace, openers) or 'endurance', so the ride was left as it is"
                )
            else:
                for field, value in bike_session_as(session, wanted, athlete.ftp_watts).items():
...
def _confirm_from_draft(
    ...
    *,
    ...
    try:
        ...
        log.warn("swallowed exception, using a default", where='backend/app/tools.py', line_hint=6356, exc_info=True)
        live_before = None
...
def _handle_replace_week_plan(input_data: dict[str, Any], *, store: StoreInterface, slug: str) -> dict[str, Any]:
    ...
    _generator_realism = _safe_realism(week.sessions) if (primary_sport == "bike" or athlete.weekly_template) else []
    ...
    if primary_sport == "bike" or athlete.weekly_template:
        ...
        week.planning_warnings = _safe_realism(
...
def _handle_patch_week_plan(input_data: dict[str, Any], *, store: StoreInterface, slug: str) -> dict[str, Any]:
    ...
    candidate.planning_warnings = _safe_realism(
...
def _handle_merge_week_plan(input_data: dict[str, Any], *, store: StoreInterface, slug: str) -> dict[str, Any]:
    ...
    **Phase 2 -- selective merge (`accept_from_proposed` given, even as an
    ...
    merged.planning_warnings = _safe_realism(
    ...
    if bool(input_data.get("confirm", False)):
        ...
        if input_data.get("event"):
            try:
                ...
                log.warn("swallowed exception, using a default", where='backend/app/tools.py', line_hint=7580, exc_info=True)
                named_event = None
