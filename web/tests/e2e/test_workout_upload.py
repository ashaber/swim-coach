...
def _open_log_tab(page):
    ...
    the feed's fetch to settle (every test in this file sees an empty feed
    -- the fixture's default workouts mock or the test's own -- so settled
    always means the empty-state message) before expanding the secondary
    manual-entry/upload section (Phase 3: "Sync from watch" is now the
    primary action; the file input lives behind the "Log manually / upload
    a file" toggle, collapsed by default -- see main.js's
    state.logManualOpen) and waiting for the file input itself. The
    settle-before-interact ordering matters just as much as before: opening
    the tab fires GET /api/workouts (main.js's loadHistory), and its
    completion render replaces the whole #app subtree via innerHTML -- a
    toggle click or file input resolved before that render can hit a
    detached node, and a change event fired on a detached node never
